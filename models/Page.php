...
class Page {
    ...
    public function read() {/* from ID so far */
        ...
        if (count($results) <= 0) {
            ...
            $respage = $results[0];
            /* Assume first result. Use splicing in searching... */
            $this -> _assoc = $respage;
            foreach ($respage as $f => $v) {
                if ($f == $this -> _id) :              $this -> id = $v;
                /* Should equal previous id... */
            ...
            /* Data not original, since we read from DB */
            $this -> _write_new = FALSE;
            /* Most likely a good idea to update existing row... */
        }
    }
